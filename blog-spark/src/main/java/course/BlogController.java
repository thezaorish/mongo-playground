    ...
    private void initializeRoutes() throws IOException {
        ...
        post("/signup", (request, response) -> {
            ...
            try {
                ...
                if (validateSignup(username, password, verify, email, root)) { // good user
                    System.out.println("Signup: Creating user with: " + username + " " + password);
                    if (!userDAO.addUser(username, password, email)) { // duplicate user
                        root.put("username_error", "Username already in use, Please choose another");
                        template.process(root, writer);
                    } else { // good user, let's start a session
        ...
        get("/newpost", (request, response) -> {
            ...
            try {
                ...
                if (username == null) { // looks like a bad request. user is not logged in
        ...
        post("/newpost", (request, response) -> {
            ...
            try {
                ...
                if (username == null) { // only logged in users can post to blog
                    response.redirect("/login");
                } else if (title.equals("") || post.equals("")) { // redisplay page with errors
                    HashMap<String, String> root = new HashMap<String, String>();
        ...
        post("/newcomment", (request, response) -> {
            ...
            try {
                ...
                if (post == null) {
                    response.redirect("/post_not_found");
                } else if (name.equals("") || body.equals("")) { // check that comment is good
        ...
        post("/login", (request, response) -> {
            ...
            try {
                ...
                if (user != null) { // valid user, let's log them in
        ...
        get("/logout", (request, response) -> {
            ...
            try {
                ...
                if (sessionID == null) { // no session to end
                    response.redirect("/login");
                } else {
